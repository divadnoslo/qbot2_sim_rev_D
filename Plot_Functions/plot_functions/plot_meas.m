...
if (p_meas_flag == true)
    ...
    hold on
    subplot(3,1,k)
    plot(t, pos(k,:) - pos_t(k,:), 'r')
    title('Meas - Truth = Error: \Deltar^t_t_b_,_x')
    xlabel('Time (s)')
    xlim([0 P.t_end])
    ylabel('r^t_t_b_,_x (m)')
%     ylim([min(pos(k,:)) - 0.1*min(pos(k,:)), ...
%           max(pos(k,:)) + 0.1*max(pos(k,:))])
    ...
    hold on
    subplot(3,1,k)
    plot(t, pos(k,:) - pos_t(k,:), 'g')
    title('Meas - Truth = Error: \Deltar^t_t_b_,_y')
    xlabel('Time (s)')
    xlim([0 P.t_end])
    ylabel('r^t_t_b_,_y (m)')
%     ylim([min(pos(k,:)) - 0.1*min(pos(k,:)), ...
%           max(pos(k,:)) + 0.1*max(pos(k,:))])
    ...
    hold on
    subplot(3,1,k)
    plot(t, pos(k,:) - pos_t(k,:), 'b')
    title('Meas - Truth = Error: \Deltar^t_t_b_,_z')
    xlabel('Time (s)')
    xlim([0 P.t_end])
    ylabel('r^t_t_b_,_z (m)')
%     ylim([min(pos(k,:)) - 0.1*min(pos(k,:)), ...
...
if (v_meas_flag == true)
    ...
    hold on
    subplot(3,1,k)
    plot(t, vel(k,:) - vel_t(k,:), 'r')
    title('Meas - Truth = Error: \Deltav^t_t_b_,_x')
    xlabel('Time (s)')
    xlim([0 P.t_end])
    ylabel('v^t_t_b_,_x (m/s)')
%     ylim([min(vel(k,:)) - 0.1*min(vel(k,:)), ...
%           max(vel(k,:)) + 0.1*max(vel(k,:))])
    ...
    hold on
    subplot(3,1,k)
    plot(t, vel(k,:) - vel_t(k,:), 'g')
    title('Meas - Truth = Error: \Deltav^t_t_b_,_y')
    xlabel('Time (s)')
    xlim([0 P.t_end])
    ylabel('v^t_t_b_,_y (m/s)')
%     ylim([min(vel(k,:)) - 0.1*min(vel(k,:)), ...
%           max(vel(k,:)) + 0.1*max(vel(k,:))])
    ...
    hold on
    subplot(3,1,k)
    plot(t, vel(k,:) - vel_t(k,:), 'b')
    title('Meas - Truth = Error: \Deltav^t_t_b_,_z')
    xlabel('Time (s)')
    xlim([0 P.t_end])
    ylabel('v^t_t_b_,_z (m/s)')
%     ylim([min(vel(k,:)) - 0.1*min(vel(k,:)), ...
...
if (a_meas_flag == true)
    ...
    hold on
    subplot(3,1,k)
    plot(t, (rpy(k,:) - rpy_t(k,:)) * 180/pi, 'r')
    title('Meas - Truth = Error: Roll (\Delta\phi^t_t_b)')
    xlabel('Time (s)')
    xlim([0 P.t_end])
%     ylim([min(rpy(k,:) * 180/pi) - 0.1*min(rpy(k,:) * 180/pi), ...
%           max(rpy(k,:) * 180/pi) + 0.1*max(rpy(k,:) * 180/pi)])
    ...
    k = 2;
    subplot(3,1,k)
    plot(t, (rpy(k,:) - rpy_t(k,:)) * 180/pi, 'g')
    title('Meas - Truth = Error: Pitch (\Delta\theta^t_t_b)')
    xlabel('Time (s)')
    xlim([0 P.t_end])
    ylabel('\theta^t_t_b (\circ)')
%     ylim([min(rpy(k,:) * 180/pi) - 0.1*min(rpy(k,:) * 180/pi), ...
%           max(rpy(k,:) * 180/pi) + 0.1*max(rpy(k,:) * 180/pi)])
    ...
    k = 3;
    subplot(3,1,k)
    plot(t,(rpy(k,:) - rpy_t(k,:)) * 180/pi, 'b')
    title('Meas - Truth = Error: Yaw (\Delta\psi^t_t_b)')
    xlabel('Time (s)')
    xlim([0 P.t_end])
    ylabel('\psi^t_t_b (\circ)')
%     ylim([min(rpy(k,:) * 180/pi) - 0.1*min(rpy(k,:) * 180/pi), ...
